 ...
 * it is an ascii, binary, XML, HDF4, or HDF5 product, has a unique description in the internal Data Dictionary of the
 * CODA library. Each of those descriptions is refered to as a CODA type (which is of type #coda_Type).
 ...
 *
 * The Data Dictionary is able to deal with many dynamic properties that can be encountered in product files.
 ...
 *
 * CODA also provides mappings of the datatypes that are available in XML, HDF4, and HDF5 products to Data Dictionary
 * specific types. When accessing HDF products via CODA you will therefore get the 'CODA view' on these files.
 ...
 *
 * More information about the Data Dictionary types and descriptions of the supported products can be found in the Data
 * Dictionary documentation that is included with the CODA package.
 */
...
/** Get the best native type for reading data of a Data Dictionary type.
 * The native type that is returned indicates which storage type can best be used when reading data of this
 * Data Dictionary type to memory. Compound types (arrays and records) that can be read directly (using a raw byte
 ...
 * returned).
 * The unit information is a string with the same text as can be found in the unit column of the Data Dictionary
...
/** Get the Data Dictionary type for a record field.
 * If the type is not a record class the function will return an error.
 * \param type CODA type.
 ...
 * entry in \a dim to the constant value for that dimension as stored in the data dictionary.
 * Variable dimension sizes can only occur when the data dictionary backend is used.
 * If the type is not an array class the function will return an error.
 * \param type CODA type.
 * \param num_dims Pointer to the variable where the number of dimensions will be stored.
