...
if (HDF5_LIBRARY)
  ...
  # program it creates to check for the library. With these default
  # values come the /MD flags, causing the linker to generate
  # LNK4098 errors about multiply defined symbols (because the
  # actual HDF5 library we are linking against is of course *NOT*
  # compiled with /MD). After experimenting a lot I come to the
  # conclusion that this is unfixable: check_library_exists does
  # not allow the possibility to configure the cmake call it makes
  # (the underlying try_compile() function does, but that is not
  # exposed), so we can neither instruct the compiler to stop using
  # /MD, nor instruct the linker to use /NODEFAULTLIB.
  #
  # I will get advice on this issue from the Cmake forums, but for
  # now I am just leaving out the library checks: I will simply
  # assume the library is the correct one.
  #
  # 
  ...
  #
  # So instead of the above stanza, we just do:
  #
  set(HAVE_HDF5 1)
